...
export function etherscan(): EtherscanApiClient {
  if (!etherscanInstance) {
    ...
    etherscanInstance = EtherscanApiClient.newInstance(
      1,
...
class EtherscanApiClient {
  ...
  async getLogEvents(
    ...
    fromBlock: number,
    topic0?: string
  ): Promise<Array<EtherscanLogEvent>> {
    let query = { address, fromBlock, toBlock: 'latest', topic0 };
